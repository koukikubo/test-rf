module Rf
  module Builders
    class TransitionBuilder
      ...
      def call
        current_base_date = Rf::Shared::BaseDate.resolve(nil)
        previous_base_date = Rf::Shared::BaseDate.previous_month(current_base_date)
        # 当月と今月のラベルを作成する関数
        ...
        Customer.includes(:reservations).find_each do |customer|
          customer_reservations = Rf::Shared::ReservationFilter.call(
            customer.reservations,
            current_base_date
          )
          previous_reservations = Rf::Shared::ReservationFilter.call(
            customer.reservations,
            previous_base_date
          )
        
          current_result = Rf::Calculators::RankRule.call(
            reservations: customer_reservations,
            base_date: current_base_date
          )

          previous_result = Rf::Calculators::RankRule.call(
