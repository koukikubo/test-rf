class RfTransitionBuilder
  ...
  def call
    ...
    reservations = Reservation.where(visit_at: period[:start]..period[:end])
    ...
    # 予約情報をもとに、行と列のキーを判定して、セルの集計を行う関数
